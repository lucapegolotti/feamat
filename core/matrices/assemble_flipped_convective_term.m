function [C] = assemble_flipped_convective_term( fespace,u_old, component, component_2 )
...
%             u1 = (transfgrad*u_old(indices)); % d_x(u1) & d_y(u1)
%             u2 = (transfgrad*u_old(indices + n_nodes)); % d_x(u2) & d_y(u2)
            grad_u = (transfgrad*u_old(indices + n_nodes * (component - 1) ));

            convective_elements = dettransf*transfun*grad_u(component_2)*(transfun')*weights(j)/2;
